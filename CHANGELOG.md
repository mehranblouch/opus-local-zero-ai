...
- **Style** — `set_style` sets `captionColor`, `highlightColor`, `captionPosition`, `uppercase`. These are render settings, not script edits — this is the only way to change caption colour, which is not present in the EditingScript at all.
